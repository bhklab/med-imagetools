...
class Pipeline:
    ...
    def __init__(
        ...
        if self.missing_strategy not in ["drop", "pass"]:
            raise ValueError(
                f"missing_strategy must be either of 'drop' or 'pass', got {missing_strategy}"
            )
    ...
    def _get_loader_subject_ids(self):
        ...
        if not all_subject_ids:
            raise AttributeError(
                "Pipeline must define at least one input op (subclass of ops.BaseInput)"
            )

        result = []
        for subject_id in unique_subject_ids:
            if not all(
                (subject_id in subject_ids for subject_ids in all_subject_ids)
            ):
                # TODO give more details about which input data is missing
                message = (
                    f"Subject {subject_id} is missing some of the input data "
                )
    ...
    def _process_wrapper(self, subject_id):
        try:
            ...
            message = (
                f"{type(e).__name__} while processing subject {subject_id}: "
                + str(e)
            )
    ...
    def run(self):
        ...
        Parallel(n_jobs=self.n_jobs, verbose=verbose)(
            delayed(self._process_wrapper)(subject_id)
...
class SequentialPipeline(Pipeline):
    def __init__(self, ops_list):
        ...
        self.sink = (
            ops_list.pop() if isinstance(ops_list[-1], BaseOutput) else None
        )
