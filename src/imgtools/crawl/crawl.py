...
def parse_dicom(dcm_path: pathlib.Path, top: pathlib.Path) -> Dict[str, str]:
	...
	try:
		...
		try:
			...
			logger.exception(
				'Error parsing main DICOM elements', exc_info=e, extra={'path': dcm_path}
			)
			raise e
		...
		subseries_dict = {
			...
			'ImageOrientationPatient': _get_meta_attribute(meta, 'ImageOrientationPatient'),
			'AnatomicalOrientationType': _get_meta_attribute(meta, 'AnatomicalOrientationType'),
			'repetition_time': _get_meta_attribute(meta, 'RepetitionTime', float, default=''),
			'echo_time': _get_meta_attribute(meta, 'EchoTime', float, default=''),
			'scan_sequence': _get_meta_attribute(meta, 'ScanningSequence', default=''),
			'mag_field_strength': _get_meta_attribute(
				meta, 'MagneticFieldStrength', float, default=''
			),
			'imaged_nucleus': _get_meta_attribute(meta, 'ImagedNucleus', default=''),
		}

		return {
			patient: {
				study: {
					# 'StudyDescription': _get_meta_attribute(meta, 'StudyDescription'),
					series: {
						# 'SeriesDescription': _get_meta_attribute(meta, 'SeriesDescription'),
						subseries: subseries_dict,
					},
				}
			}
		}
