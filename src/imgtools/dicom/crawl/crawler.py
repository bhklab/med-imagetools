...
@dataclass
class Crawler:
    """Crawls a DICOM directory, searching recursively to identify 
    all DICOM files and extract key metadata such as:
    ...
    This information is compiled into an index that serves as a map of the entire directory.

    Parameters
    ----------

    dicom_dir: Path
        Path of the directory to crawl.
    output_dir: Path, default=None
        Path of the directory to save output.
    dataset_name: str | None, default=None
        Name of the output folder generated by crawler. 
        If None, the output folder name is the same as the input folder name.
