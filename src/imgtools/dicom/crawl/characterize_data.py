...
def file_path(path: str) -> Path:
    ...
    if path_obj.is_dir():
        raise argparse.ArgumentTypeError(
            f"Invalid argument ({path}), not a file path."
        )
    return path_obj
...
def load_optional_parameters(
    file_name: Union[str, Path], parser: argparse.ArgumentParser
) -> Dict[str, Any]:
    """
    ...
    file_name : Union[str, Path]
        Name of JSON configuration file.
    parser : argparse.ArgumentParser
        Parser for the optional commandline parameters.

    Returns
    -------
    Dict[str, Any]
        Dictionary containing the parsed configuration options
    """
    config_argv = []
    ...
    config_data, additional_args = parser.parse_known_args(
        config_argv, namespace=None
    )

    if additional_args:
        logger.warning(
            f"Unexpected arguments found in configuration file ({additional_args})."
        )
...
def inspect_grayscale_image(
    sitk_image: sitk.Image, image_info: Dict[str, Any]
) -> None:
    """
    Extract relevant intensity information from a grayscale image.

    Parameters
    ----------
    sitk_image : sitk.Image
        A SimpleITK grayscale image
...
def inspect_image(
    ...
    ----------
    sitk_image : sitk.Image
        Input image for inspection, 2D or 3D.
    image_info : Dict[str, Any]
        Image information is added to this dictionary (e.g. image_info["image size"] = "(512,512)").
    ...
    meta_data_info : Dict[str, str]
        The meta-data information whose values will be reported.
    ...
    thumbnail_settings : Dict[str, Any]
        Dictionary containing the following keys "thumbnail_sizes", "projection_axis" and
    ...
    if (
        ...
        image_info["pixel type"] = (
            sitk_image.GetPixelIDTypeAsString() + " gray"
        )
    ...
    if thumbnail_settings:
        image_info["thumbnail"] = image_to_thumbnail(
            sitk_image, **thumbnail_settings
        )
...
def inspect_single_file(
    ...
    imageIO: str = "",
    meta_data_info: Dict[str, str] = {},
    external_programs_info: Dict[str, str] = {},
    thumbnail_settings: Dict[str, Any] = {},
) -> Dict[str, Any]:
    """
    ...
    file_name : Union[str, Path]
        Image file name.
    imageIO : str, optional
        Name of image IO to use. To see the list of registered image IOs use the
    ...
    meta_data_info : Dict[str, str], optional
        The meta-data information whose values will be reported.
        Dictionary structure is description:meta_data_tag
        (e.g. {"radiographic view" : "0018|5101", "modality" : "0008|0060"}).
    external_programs_info : Dict[str, str], optional
        A dictionary of programs that are run with the file_name as input
    ...
    thumbnail_settings : Dict[str, Any], optional
        A dictionary containing the settings required for creating a 2D thumbnail.
    ...
    -------
    Dict[str, Any]
        Dictionary with the following entries: files, MD5 intensity hash,
    ...
    try:
        ...
        for k, p in external_programs_info.items():
            try:
                ...
                subprocess.run(
                    [p, str(file_name)], check=True, capture_output=True
                )
...
def inspect_files(
    ...
    ----------
    root_dir : Union[str, Path]
        Path to the root of the data directory. Traverse the directory structure
        and inspect every file (also report non image files, in which
        case the only valid entry will be the file name).
    max_processes : int
        Maximal number of processes to use in when performing parallel processing.
    ...
    -------
    pandas.DataFrame
        Each row in the data frame corresponds to a single file.
    """
    root_path = Path(root_dir)
    all_file_names = []
    for dir_path, _, file_names in os.walk(root_path):
        all_file_names += [
            Path(dir_path).absolute() / fname for fname in file_names
        ]
    ...
    with concurrent.futures.ProcessPoolExecutor(max_processes) as executor:
        ...
        for file_name, future in tqdm(
            ...
            try:
                ...
                logger.error(f"Failed process for {file_name}: {e}")
    return pd.DataFrame.from_dict(res)


def inspect_single_series(
    series_data, meta_data_info={}, thumbnail_settings={}
):
    """
...
def get_series_key_fname(
    file_name: Union[str, Path], additional_series_tags: List[str]
) -> Tuple[str, str]:
    """
    ...
    file_name : Union[str, Path]
        Name of file.
    additional_series_tags : List[str]
        List of DICOM tags that together with series and study UID serve to uniquely
        identify files belonging to the same series.
...
def inspect_series(
    ...
    disable_tqdm : bool
        Display a progress bar, or not.
    additional_series_tags : List[str]
        List of DICOM tags used to uniquely identify files belonging to the same
    ...
    -------
    pd.DataFrame
        Each row in the data frame corresponds to a single series.
    """
    ...
    with concurrent.futures.ProcessPoolExecutor(max_processes) as executor:
        ...
        for file_names, future in tqdm(
            ...
            try:
                ...
                logger.error(f"Failed process for {file_names}: {e}")
    return pd.DataFrame.from_dict(res)
...
def image_to_thumbnail(
    ...
    ----------
    img : sitk.Image
        A 2D or 3D grayscale or sRGB image.
    thumbnail_sizes : List[int]
        The 2D sizes of the thumbnail.
    interpolator : int
        Interpolator to use when resampling to a thumbnail. Nearest neighbor
        is computationally efficient and is applicable for
        both segmentation masks and scalar images.
    projection_axis : int
        The axis along which we project 3D images. Must be between 0 and 2.
    ...
    if img.GetNumberOfComponentsPerPixel() >= 3:
        ...
        I = (
            ...
            + I ** (1 / 2.4)
            * sitk.Cast(I > 0.0031308, sitk.sitkFloat32)
            * 1.055
            - 0.055
        )
    ...
    new_spacing = [
        ...
        for ospc, osz, nsz in zip(
            img.GetSpacing(), img.GetSize(), thumbnail_sizes
        )
    ]
...
def image_list_to_faux_volume(image_list, tile_size):
    ...
    if len(faux_volume_slices) > 1 and (
        faux_volume_slices[-1].GetHeight()
        != faux_volume_slices[-2].GetHeight()
        or faux_volume_slices[-1].GetWidth()
        != faux_volume_slices[-2].GetWidth()
    ):
...
def characterize_data(argv=None):
    ...
    parser.add_argument(
        "output_file", type=file_path, help="output csv file path"
    )
    parser.add_argument(
        "analysis_type",
    ...
    if len(args.external_applications) != len(
    ...
    if len(args.metadata_keys) != len(args.metadata_keys_headings):
        print(
            "Number of metadata keys and their headings do not match.",
            file=sys.stderr,
        )
        return 1
    ...
    if args.analysis_type == "per_file":
        df = inspect_files(
            ...
            meta_data_info=dict(
                zip(args.metadata_keys_headings, args.metadata_keys)
            ),
            external_programs_info=dict(
                zip(
                    args.external_applications_headings,
                    args.external_applications,
                )
            ),
            thumbnail_settings=thumbnail_settings,
        )
    ...
    with open(
        os.path.join(
            ...
            time.strftime("%d_%m_%Y-%H_%M_%S_")
            + "characterize_data_settings.json",
        ),
        "w",
    ) as fp:
    ...
    if not args.ignore_problems:
        df.dropna(inplace=True, thresh=2)
    image_counts = (
        df["MD5 intensity hash"].value_counts().reset_index(name="count")
    )
    duplicates = df[
    ...
    if not duplicates.empty:
        duplicates.to_csv(
            f"{os.path.splitext(args.output_file)[0]}_duplicates.csv",
            index=False,
        )
    ...
    if df["image size"].apply(lambda x: len(x) == 3 and x[2] > 1).any():
        sizes = df["image size"].apply(
            lambda x: x if len(x) == 3 else x + (1,)
        )
        ...
        cb.set_ticks(
            np.linspace(min(z_size), max(z_size), 5, endpoint=True, dtype=int)
        )

        spacings = df["image spacing"].apply(
            lambda x: x if len(x) == 3 else x + (1,)
        )
        x_spacing, y_spacing, z_spacing = zip(*spacings)
        sc = spacing_ax.scatter(
            x_spacing, y_spacing, c=z_spacing, cmap="viridis"
        )
        cb = spacing_fig.colorbar(sc)
        cb.set_label(
            "z spacing [mm]", rotation=270, verticalalignment="baseline"
        )
    ...
    # convert faux 3D sizes to 2D by removing the last dimension.
        ...
        spacings = df["image spacing"].apply(
            lambda x: x if len(x) == 2 else x[0:2]
        )
