...
class DataGraph:
    ...
    def form_graph(self) -> None:
        ...
        df_filter.loc[
            (df_filter.reference_rs_x.isna()) & (~df_filter.reference_rs_y.isna()),
            "reference_rs_x",
        ] = df_filter.loc[
            (df_filter.reference_rs_x.isna()) & (~df_filter.reference_rs_y.isna()),
            "reference_rs_y",
        ...
        self.df_edges.loc[self.df_edges.study_x.isna(), "study_x"] = self.df_edges.loc[
            self.df_edges.study_x.isna(), "study"
        ]
        self.df_edges.drop(
            columns=[
                "study_y",
    ...
    def _form_edges(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for edge in range(8):
            ...
            if edge == 0:
                df_combined = pd.concat(
                    [
                        ...
                        pd.merge(
                            struct, dose, left_on="series", right_on="reference_rs"
                        ),
                    ]
                ).drop_duplicates(
            ...
            # FORMS RTSTRUCT->RTPLAN on ref_rs
                ...
                df_combined = pd.merge(
                    struct, plan, left_on="instance_uid", right_on="reference_rs"
                )
        ...
        for col in final_df.columns:
            if col.startswith("folder"):
                final_df[col] = final_df[col].apply(
                    lambda x: (self.edge_path.parent.parent / x).resolve().as_posix()
                    if isinstance(x, str)
                    else x
    ...
    def graph_query(
        ...
        for bad in change_df:
            ...
            col_ids = [
                cols for cols in list(final_df.columns)[1:] if bad != cols.split("_")[1]
            ]
        ...
        * Oct 11th, 2022: Reverted to studyID-based loop + improved readability and make CT,RTSTRUCT,RTDOSE mode pass tests
        ...
        for _, study in enumerate(rel_studyids):
            ...
            for ct in ct_series:
                ...
                temp = {"study": study, ct: {"modality": modality, "folder": folder}}
            ...
            for _, edge in rest_locs.iterrows():
                for k in range(len(ct_series)):
                    ...
                    modality_origin = (
                        "CT" if edge["modality_y"] == "RTDOSE" else edge["modality_x"]
                    )

                    key, key_series = self._check_save(
                        save_folder_comp[k], edge["modality_y"], modality_origin
                    )
