...
# from .sparsemask import SparseMask #deprecating this for now


class Segmentation(sitk.Image):
    roi_indices: dict[str, int]
    existing_roi_indices: Optional[dict[str, int]]
    num_labels: int
    metadata: dict
    ...
    def __init__(
        ...
        self.raw_roi_names = raw_roi_names or {}
    ...
    def __getitem__(self, idx) -> Segmentation | sitk.Image | Any:  # noqa
        # sitk image allows other metadata except for size ...
        if idx == "size":
            return self.GetSize()
        elif idx in self.roi_indices:
            return self.get_label(name=idx)
