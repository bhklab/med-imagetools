...
def save_data(self):
    ...
    """
    files = glob.glob(
        pathlib.Path(self.output_directory, ".temp", "*.pkl").as_posix()
    )
    for file in files:
        ...
        self.output_df.loc[subject_id, list(metadata.keys())] = list(
            metadata.values()
        )  # subject id targets the rows with that subject id and it is reassigning all the metadata values by key

    folder_renames = {}
    for col in self.output_df.columns:
        if col.startswith("folder"):
            self.output_df[col] = self.output_df[col].apply(
                lambda x: x
                if not isinstance(x, str)
                else pathlib.Path(x)
                .as_posix()
                .split(self.input_directory)[1][1:]
            )  # rel path, exclude the slash at the beginning
            folder_renames[col] = f"input_{col}"
    self.output_df.rename(
        columns=folder_renames, inplace=True
    )  # append input_ to the column name
    ...
    if (
        self.is_nnunet
    ):  # dataset.json for nnunet and .sh file to run to process it
        imagests_path = pathlib.Path(
            self.output_directory, "imagesTs"
        ).as_posix()
        images_test_location = (
            imagests_path if os.path.exists(imagests_path) else None
        )
        # print(self.existing_roi_names)
        generate_dataset_json(
            pathlib.Path(self.output_directory, "dataset.json").as_posix(),
        ...
        shell_path = pathlib.Path(
            self.output_directory, child.split("_")[1] + ".sh"
        ).as_posix()
        ...
        markdown_report_images(
            self.output_directory, self.total_modality_counter
        )  # images saved to the output directory
    ...
    with open(markdown_path, "w", newline="\n") as f:
        ...
        if self.is_nnunet:
            ...
            pie_path = pathlib.Path(
                "markdown_images", "nnunet_train_test_pie.png"
            ).as_posix()
            ...
            bar_path = pathlib.Path(
                "markdown_images", "nnunet_modality_count.png"
            ).as_posix()
            output += (
                f"![Pie Chart of Image Modality Distribution]({bar_path})\n\n"
            )
        f.write(output)
