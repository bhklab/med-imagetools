...
class VectorMask(MedImage):
    ...
    def to_label_image(self) -> Mask:
        ...
        This converts the multi-channel (vector) mask into a single-channel label image.
        Each channel index becomes its own unique label in the scalar output image,
        assigned to voxels where that channel has value 1.

        This operation assumes there is no overlap across channels. If any voxel
        has more than one active channel (i.e., multiple 1s), a ValueError is raised.

        Returns
        -------
        Mask
            A scalar label mask image (each voxel value is a label index).

        Raises
        ------
        ValueError
            If any voxel has more than one label (i.e., overlap).
        """
        arr = sitk.GetArrayFromImage(self)
