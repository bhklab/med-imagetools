...
class SegNrrdWriter(BaseWriter):
    ...
    def put(self, subject_id, mask, **kwargs):
        ...
        for n, i in enumerate(labels):
            segment_info[f"Segment{n}_Color"] = [0., 0., 0.]
            segment_info[f"Segment{n}_ColorAutoGenerated"] = '0'
            segment_info[f"Segment{n}_Extent"] = [0,0,0,0,0,0]
            segment_info[f"Segment{n}_ID"] = str(n)
            segment_info[f"Segment{n}_Name"] = i
            segment_info[f"Segment{n}_NameautoGenerated"] = '0'
            
        
        print("space metadata:", space)
        header = {'type': 'unsigned char',
                  'dimension': 4,
                  'space': space,
                  'sizes': mask.GetSize(),
                  'space directions': [[float('nan'), float('nan'), float('nan')], 
                                       [spacing[0], 0., 0.],
                                       [0., spacing[1], 0.],
                                       [0., 0., spacing[2]]],
        ...
        mask = np.transpose(sitk.GetArrayFromImage(mask), [-1-i for i in range(len(mask.GetSize())+1)]) # permute axes to original orientations
        nrrd.write(out_path, mask, header=header, compression_level=self.compression_level, **kwargs)
        print(out_path)
