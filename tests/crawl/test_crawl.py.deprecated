...
def test_shared_datadir_datasets(data_paths: dict[str, Path], dataset_key) -> None:
    """
    ...
    assert crawl_path.with_suffix(".json").exists(), (
        f"Expected crawl file {crawl_path.with_suffix('.json')} not found"
    )
    ...
    for modality, count in modality_counts.items():
        assert modality in COLLECTIONS[dataset_key], (
            f"Modality {modality} not found in COLLECTIONS"
        )
        assert count == COLLECTIONS[dataset_key][modality], (
            f"Expected {COLLECTIONS[dataset_key][modality]} {modality} files, found {count}"
        )
